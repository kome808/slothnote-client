...
#### 3c. 搜尋關聯筆記
先使用「摘要優先」策略搜尋關聯，降低 token 消耗：

1. 先用 `grep_search` 在 `notes/` 中搜尋 `## 一句話摘要` 區塊與 frontmatter（title/category/tags）。
2. 只讀取候選筆記的前半段（frontmatter + `## 一句話摘要` + `## 關鍵重點`），先做初步關聯判斷。
3. 對高相關候選再讀取全文，補強洞察內容；讀取篇數依相關性動態調整，不設固定上限。
...
#### 3e. 自動延伸討論（每篇都做）
寫完單篇筆記後，立即補上「## 延伸討論」內容，不需再手動執行 `/extend-discussion`：

- 先以本篇的摘要、關鍵重點、AI 洞察為基礎。
- 再參考 3c 找到的高相關歷史筆記，產生跨文整合觀點。
- 內容至少包含：
  - 與既有筆記的呼應或矛盾
  - 知識缺口（目前還缺什麼）
  - 下一步建議（可追蹤的行動）

若本篇找不到關聯筆記，仍要產生延伸討論（可聚焦在「待驗證假設」與「後續觀察指標」）。
...
同步後會自動在你設定的 Notion 頁面下刷新各分類頁，點進分類即可看到文章表格列表。
